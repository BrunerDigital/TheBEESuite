...
Baseline established September 11; evidence reconciled September 13, 2026. Parent and Teacher are the only intended v1 native apps. This record separates unsigned compilation from signed/device/store readiness. Current web changes and technical work remain tracked in the [school UI release index](SCHOOL_UI_RELEASE_EVIDENCE_2026-09-13.md).
...
The September 12 messaging privacy correction adds per-category semantic validation and compiled/source reconciliation for both SDKs. The latest successful main evidence is [run 34705460891](https://github.com/BrunerDigital/TheBEESuite/actions/runs/34705460891), source `04a0526d96daddb87c1d91daae7a4304e35cb474`. Parent job `103584593775` and Teacher job `103584593940` each passed all seven checks with Xcode 26.6 (`17F113`), device/simulator SDK 26.5 and an isolated iPhone 17/iOS 26.5 simulator. Both compiled unsigned Release builds for device and simulator, reconciled their compiled privacy/resources, and passed public cold/terminated-relaunch liveness and sign-in-heading recognition. Device and simulator products include private messaging, tracking false, and 17 Parent / 14 Teacher privacy categories. This run does verify the corrected manifests; the preceding run remains historical.

On September 13 the exact main-run artifacts were downloaded and all four public sign-in screenshots visually inspected. Parent artifact `10301404607` has GitHub digest `sha256:9593d5cbe764211e4c8ffe7cdad28b33e11352b946a47a9bb2332acce72570d9`; Teacher artifact `10301830318` has digest `sha256:fb4276a3d3550f1757597a14bdb1bca96a57bb6df3d921a5388d90bd40b3ccd0`. GitHub expiry is September 26; retained reports, screenshots and logs are under `output/ios-native/main-04a0526d-20260913/{parent,teacher}/`. Reports explicitly mark signing, archive, upload, authenticated native flows, physical devices and store-ready screenshots false. No credentials or private records appear in the public captures.

Git-object comparison found native projects, shells, Capacitor configuration, dependency/lockfile, workflow, verifier/sync/store scripts and native tests unchanged between that source and current web main `f1f1ca21f8345cc73b430e3d0913a65435a37213`. This is identical-native-source evidence, not an exact-SHA native run or proof of later authenticated web behavior inside WKWebView. At submission freeze, rerun the existing unsigned workflow once on the final approved SHA. No Mac project/host is currently connected and local Xcode/Swift are unavailable. Record the exact SHA, workflow URL, Xcode/SDK/runtime, compile results and screenshot review for every native release. CI never archives, signs, uploads, invites testers or submits either app.
