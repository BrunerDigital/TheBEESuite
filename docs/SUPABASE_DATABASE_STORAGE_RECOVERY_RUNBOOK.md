...
9. Before reconnecting production providers, cron schedules, webhooks, or background writers, reconstruct the post-snapshot external-event timeline from provider records and independent audit evidence. Reconcile **all restored Stripe payments and invoices regardless of current status**, including PaymentIntents, captures, refunds, disputes, reversals, payouts, connected-account context, and their application ledger and family-balance effects. A pre-snapshot `PAID` invoice may have been refunded or disputed later, and an `OPEN` invoice may have been paid later. Restore or explicitly account for every missing financial event before accepting balances as current. Separately check every open/due invoice eligible for autopay or saved-method collection, its consent and authorization, and record an exact collect, hold, or already-paid disposition with the billing owner. Autopay selects open invoices dynamically and can create a fresh payment and idempotency key after restore; keep automatic collection disabled until every candidate is resolved and a second charge is ruled out.

   Inventory every restored scheduled or pending outbound item, including campaigns, email/SMS, web push, integration deliveries, and payment-related jobs. Compare each item with post-snapshot provider receipts, current delivery state, and application audit history. Apply the same post-snapshot reconciliation to other jobs that select work dynamically rather than from a pending queue. Record an exact-item cancel, suppress, or replay decision; preserve delivery and financial history and require the separate real-send or money approval gate for any replay. Verify that items completed after the restore point cannot dispatch a second time and that approved pending items retain their intended recipients and idempotency keys. If provider events, a queue, or a dynamic candidate set cannot be reconciled, keep its worker and provider credentials disabled and the target out of production. A restored `pending`, `scheduled`, `OPEN`, or `PAID` status alone does not establish what happened after the snapshot.
