...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const printMode = new URL(request.url).searchParams.get("mode") === "print";
  const signedUrl = await createAssetHubSignedUrl(
    asset.storageKey,
    printMode ? undefined : metadata.originalName,
  );
  await writeAuditLog(user, {
    action: printMode ? "asset_hub.print_opened" : "asset_hub.downloaded",
    resource: "BrandAsset",
    resourceId: asset.id,
    metadata: { name: metadata.originalName },
  });
