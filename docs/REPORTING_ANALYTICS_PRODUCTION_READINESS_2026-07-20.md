...
4. **Exports lack full traceability metadata.** Analytics CSV contains the title and rows but not generated-at, selected date range, or selected school scope. PDF has generated-at but not the explicit filter scope and may truncate. FTE CSV similarly lacks generated-at and filter metadata. Owner: **Brenden**. Exact retest: add a machine-readable metadata section or companion manifest and verify the exported scope against the UI.
5. **Freshness service levels are not approved.** Analytics is computed at request time from the application database, while the external Google Sheet FTE snapshot depends on source availability and the internal weekly FTE workflow has a Friday noon ET deadline. There is no approved stale-data threshold or named exception response. Owner: **Brenden**. Exact retest: approve freshness targets per report, expose source-updated timestamps where available, and test stale/error states.
