...
test("an updated service worker reloads an open portal with loop protection", () => {
  ...
  assert.match(installManager, /Date\.now\(\) - lastReloadAt < reloadWindowMs/);
  assert.match(installManager, /sessionStorage\.setItem\(reloadKey, String\(Date\.now\(\)\)\)/);
