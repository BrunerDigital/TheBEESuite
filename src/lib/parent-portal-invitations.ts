...
export function buildParentPortalInvitationText({
  ...
  const billingSteps = transitioningFromProcare && billingCutoverApproved
    ? [
        ...
        "7. Choose an available card or bank option, complete the secure Stripe steps, and wait for the confirmation before leaving or trying again.",
      ]
    : [
...
export function buildParentPortalInvitationHtml({
  ...
  const paymentStep = transitioningFromProcare && billingCutoverApproved
    ? `<li><strong>Review an approved tuition invoice:</strong> open Billing, confirm the family, service period, amount, credits, and due date, then choose an available card or bank option. Complete the secure Stripe steps and wait for confirmation before leaving or trying again.</li>`
    : `<li><strong>Set up payments when your school enables them:</strong> open Billing, choose Set Up Card Autopay or Set Up Instant Bank, complete the secure Stripe form, and return to the portal to confirm the saved method. If checkout is not active yet, stop and contact the school.</li>`;
  return `<!doctype html>
<html>
