...
9. Before reconnecting production providers, cron schedules, webhooks, or background writers, inventory every restored scheduled or pending outbound item, including campaigns, email/SMS, web push, integration deliveries, and payment-related jobs. Compare each item with post-snapshot provider receipts, current delivery state, and application audit history. Also reconcile **every restored open/due invoice eligible for autopay or saved-method collection**, its consent and payment authorization, and any post-snapshot Stripe PaymentIntent, charge, refund, payout-account context, and application ledger entry. Autopay selects open invoices dynamically and can create a fresh payment and idempotency key after restore; queue reconciliation alone cannot prevent a second charge. Record an exact-invoice collect, hold, or already-paid disposition with the billing owner, and keep automatic collection disabled until every candidate is resolved and a second charge is ruled out. Apply the same post-snapshot reconciliation to other jobs that select work dynamically rather than from a pending queue. Record an exact-item cancel, suppress, or replay decision for remaining outbound items; preserve delivery and financial history and require the separate real-send or money approval gate for any replay. Verify that items completed after the restore point cannot dispatch a second time and that approved pending items retain their intended recipients and idempotency keys. If a queue or dynamic candidate set cannot be reconciled, keep its worker and provider credentials disabled; do not infer that a restored `pending`, `scheduled`, or `OPEN` status means the action never happened.
