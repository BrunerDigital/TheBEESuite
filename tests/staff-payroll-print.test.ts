...
test("print reports remove hidden dashboard layout instead of leaving blank pages", async () => {
  ...
  assert.match(source, /\*:has\(\.bee-print-report-active\) \{[\s\S]*overflow: visible !important/);
  assert.match(source, /\*:has\(\.bee-print-report-active\) \{[\s\S]*max-height: none !important/);
