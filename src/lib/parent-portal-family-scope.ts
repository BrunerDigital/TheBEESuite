...
export function resolveParentPortalFamilyScope(
  guardians: Array<{ id: string; familyId: string }>,
): ParentPortalFamilyScope {
  const familyIds = Array.from(new Set(guardians.map((guardian) => guardian.familyId)));
...
export async function getParentPortalFamilyScope(userId: string) {
  ...
  return resolveParentPortalFamilyScope(guardians);
}
