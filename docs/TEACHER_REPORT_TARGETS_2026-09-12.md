...
## Protected review correction

PR #367 review found a real QA lifecycle gap: browser launch occurred before cleanup, so a missing browser binary could leave the local fixture server listening. A small shared `withFixtureBrowser` helper now owns launch, context/test setup and teardown. It closes the browser and owned fixture server on failure; a browser-close rejection still closes the server and its connections. This changes test tooling only, not product behavior or authorization.

Four new regressions cover failed launch, failed context/setup, failed browser cleanup and the actual harness's cleanup boundary. Focused checks pass 6/6. An actual run with a deliberately nonexistent task-local `PLAYWRIGHT_BROWSERS_PATH` exits with the expected status 1 in about four seconds, rather than hanging; no browser installation or deletion was needed. The normal Chromium and WebKit harnesses each pass all 29 cases again. The corrected full gate passes Prisma generation, lint (zero errors; one pre-existing unrelated warning), types, all 2,161 tests and optimized build (`output/wave14-review-vercel-build.log`). Protected CI and live verification remain pending for this correction.

## Released and verified — September 12, 20:25 UTC

PR #367 passed corrected-head CI `34716387821`, CodeQL `34716386234`, exact-head Ready preview `dpl_BrkSGLxht9rp9iXxm5a2vYpVtRWh` and its resolved protected review conversation. It merged at 20:19:07 UTC to main `8687feaf040718af614496e1dbc7aa460901a31c` without bypass or force push. Production `dpl_F9T8mLieVBcqDjiuBDMz7JvpNUhq` is Ready on that exact commit and all five canonical aliases. Health/database connected at 20:23:34 UTC. Build error inspection found no failures; scoped post-ready error/fatal and 5xx counts remained empty after verification.

Fresh strictly guarded App Review fake-account checks completed at 20:25:50 UTC: all 20 Parent/Teacher phone/desktop routes, 52 Parent fit checks, eight shared-shell cases, four Teacher navigation cases, four message history/denial checks, four message layouts and existing profile/document/home checks pass. New report-target checks pass 12/12 across 320/390px and 100/200% text, including all 13 fake roster recipients, complete read-only disclosure, one-child selection, full names, 44px targets and no horizontal overflow. Two matched logins, 57 verified heartbeats, six empty billing-denial probes, two read-only payment-status and three history requests were allowed. Zero product writes, blocked requests, HTTP errors or client exceptions. Evidence: `output/playwright/app-review-production-after-teacher-report-targets/results.json` and `output/wave14-production-verification.log`.

The refreshed 12-page printable fake-data browser packet is `output/pdf/BEE_Suite_Current_Mobile_UI_2026-09-12_PR367.pdf`, captured during this verified release. It is not native, physical-device, signing, archive, TestFlight or App Store evidence. Task-local child selection in Incident/Location and expanded Photo remains the separate subsequent wave.
