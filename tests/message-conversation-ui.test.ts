...
test("director inbox keeps school-scoped threads visible after a family has no current enrollment", () => {
  assert.match(routePage, /const directorMessageCenterIds = user\.role === UserRole\.CENTER_DIRECTOR \|\| user\.role === UserRole\.ASSISTANT_DIRECTOR[\s\S]*?\[user\.primaryCenterId\]/);
