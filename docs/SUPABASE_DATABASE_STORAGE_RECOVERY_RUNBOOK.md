...
6. Reconcile database object references to the manifest and compare source Storage metadata with the API-created target metadata. Verify representative tenant/center/family/child relationships, object counts, sizes and SHA-256 values, private bucket settings, short-lived signed access, and denial of direct unauthenticated access. Compare source and target Auth users and identities by their internal stable IDs without exposing password hashes. Separately perform a complete bidirectional reconciliation of normalized emails between Auth users and Prisma `User` records expected to sign in; account for inactive, invitation-only, and other intentionally excluded records, and check scoped grants for every matched active user. The application login resolves the Prisma record by email, not by a stored Auth user ID. Prove an approved test account can authenticate through the target Auth service and resolve to the intended application user and school, and that another school's data stays denied. Record whether existing Supabase sessions must re-authenticate under the target's signing configuration. A complete row count without a working login or required Storage metadata is a failed recovery drill.
7. Before any **production re-entry**, invalidate application sessions restored from the older database snapshot. The `bee_suite_session` cookie is signed with the application's `AUTH_SECRET` and checked against Prisma `User.sessionVersion` and `DeviceSession`; a Supabase JWT change alone does not revoke it. Under the named stop authority, review and execute a tracked target-only transaction that increments every restored user's session version and revokes every still-active restored device session without overwriting earlier revocation history. Verify a pre-restore cookie is denied and a fresh approved login works with the intended grants. Reconcile any access or revocation decisions made after the selected restore point before reconnecting users. If invalidation or reconciliation cannot be proven, keep the restored target isolated.
8. Record achieved RPO/RTO, exceptions, missing writes, reconciliation ownership, and technical review. Under the approved retention plan, destroy the disposable target or clear the restored rehearsal data from a retained lab and verify that no production Auth users, application records, or object bytes remain. Production re-entry requires the named human stop authority.
