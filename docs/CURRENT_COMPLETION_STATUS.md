...
4. Restore into an isolated target with outbound jobs and provider side effects disabled. Because Recovery Lab is already migrated, loading data directly would execute application triggers unless the restore order or a reviewed transaction-scoped suppression procedure prevents it; re-enable triggers and explicitly check foreign keys and business invariants afterward. Then verify record relationships, counts, financial and audit history, private object hashes, access isolation, and measured RPO/RTO. The current Recovery Lab has schema only. No production data has been copied and a full restore has not been claimed.
