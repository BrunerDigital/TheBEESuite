...
export function fullyVoidedTuitionChargeIds(billingAccountId: string, invoices: InvoiceEvidence[]) {
  ...
  for (const invoice of invoices) {
    ...
    const credits = entries.filter(entry => entry.type === "tuition_credit" && entry.sourceSystem === "bee_suite" && entry.amountCents < 0);
    const voids = entries.filter(entry => entry.type === "invoice_void" && entry.sourceSystem === "bee_suite_manual" && entry.amountCents < 0);
    if (!charges.length || voids.length !== 1 || charges.length + credits.length + voids.length !== entries.length) continue;
