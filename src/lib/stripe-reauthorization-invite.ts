export const STRIPE_REAUTHORIZATION_SUBJECT_PREFIX = "Action required: Finish your school's Stripe account setup";
...
export function buildStripeReauthorizationInvite(input: {
  ...
  const text = [
    ...
    explanation,
    "",
    "Please have an authorized business representative complete the secure steps for the school. The BEE Suite login and Stripe login are separate. The BEE Suite never receives or stores the Stripe password. You may be asked to confirm the legal business name, tax information, representative details, and payout bank.",
    "",
    accuracyNote,
    "",
...
<h1 style="font-size:26px;line-height:1.25;margin:18px 0 14px">Finish the existing Stripe account for ${escapeHtml(schoolName)}</h1>
