...
2. **Database migration history and held repairs** use the Supabase migration registry as the sole production writer. The agency receivable and reconciliation migrations are already applied and verified in production under Supabase versions `20260908161144` and `20260908161154`; do not apply the former local aliases `20260903190000` or `20260903210000` again. Production's Supabase and Prisma registries use different version identities for those same reviewed SQL bytes, so `npm run db:migrate`, `prisma migrate deploy`, `prisma migrate resolve`, manual SQL execution, and migration-ledger edits remain prohibited. The unapplied `20260908200500_agency_child_scope_guard_repair` stays in `supabase/held-migrations/`, outside automatic deployment, until its exact production migration is separately authorized. After authorization, move that unchanged file into `supabase/migrations/` in a dedicated protected PR, verify its hash against the Prisma mirror, re-run preflight and backup evidence, and let only the approved Supabase main-branch path apply it. If any authorized future phase partially applies or fails, keep the operational freeze and any committed database fence in place, capture the Supabase error, registry state, and exact catalog evidence, and use a separately reviewed migration with a new identity as a forward repair. Never edit the original migration bytes, never reuse either original name/version for altered SQL or object definitions, never mark a migration successful by hand, and never insert or edit a migration-history row or switch migration writers.
