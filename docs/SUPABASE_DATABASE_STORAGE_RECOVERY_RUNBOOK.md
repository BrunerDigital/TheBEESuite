...
6. Reconcile database object references to the manifest and compare source Storage metadata with the API-created target metadata. Verify representative tenant/center/family/child relationships, object counts, sizes and SHA-256 values, private bucket settings, short-lived signed access, and denial of direct unauthenticated access. Compare source and target Auth users and identities by stable ID without exposing password hashes; reconcile every restored application user that should sign in to the corresponding Auth identity and scoped grants. Prove an approved test account can authenticate through the target Auth service and resolve to the intended application user and school, and that another school's data stays denied. Record whether existing sessions must re-authenticate under the target's signing configuration. A complete row count without a working login or required Storage metadata is a failed recovery drill.
