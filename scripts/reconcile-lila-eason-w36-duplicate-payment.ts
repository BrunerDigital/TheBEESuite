...
async function main() {
  ...
  if (!before.refunded) {
    const refunded = await issueFamilyRefund(actor, { familyId: FAMILY_ID, amountCents: 6_000, reason: REASON, preferredPaymentIds: [DUPLICATE_PAYMENT_ID], restrictToPaymentIds: [DUPLICATE_PAYMENT_ID], operationId: `lila-eason-w36-duplicate:${DUPLICATE_INVOICE_ID}`, tenantId: dbUser.tenantId });
