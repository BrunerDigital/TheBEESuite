...
test("ProCare imports require the exact completed review before commit", () => {
  ...
  assert.match(panel, /File names and extensions do not matter/);
  assert.match(panel, /multiple/);
  assert.match(panel, /identifies enrollment, parent, relationship, and child-information data/);
  ...
  assert.match(panel, /Select the four ProCare reports together/);
