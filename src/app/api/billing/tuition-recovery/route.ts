...
async function POSTHandler(request: NextRequest) {
  ...
  const previewDueChildren = typeof body.previewDueChildren === "number" && Number.isFinite(body.previewDueChildren)
    ...
    : null;
  const previewCenterId = clean(body.previewCenterId);
  ...
  if (!dryRun && previewCenterId !== center.id) {
    return NextResponse.json({
      ok: false,
      error: "Previewed school changed before invoices were created. Run the preview again before creating invoices.",
      centerId: center.id,
      previewCenterId: previewCenterId || null,
    }, { status: 409 });
  }
