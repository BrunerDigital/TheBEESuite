...
- Supabase Auth/session-cookie login, forgot/reset password, role-specific first-login handling, school-issued parent invitation credentials with optional later password change, teacher login generation, login rate limiting, session revocation, and device-session controls.
...
- CRM inquiry intake, lead routing, lead notes/tasks/messages/tours, scoped lead visibility, pipeline stages, public inquiry embed, trusted-origin CORS that cannot be replaced by environment overrides, UTM/source capture, and Google Sheet backup/snapshot foundations.
...
- ProCare import page/API with unencrypted CSV support, batch/row tracking, family/guardian/child/classroom/staff/invoice/balance/attendance import foundations, preview/diff, rollback/export backup, duplicate matching controls, staff login generation, migration runbook, ignored local preparation packages, and a source-coverage manifest retained through preparation.
...
- Billing foundations for billing accounts, invoices, invoice items, payments, ledger entries, products, school-scoped tuition plans, canonical child-level weekly tuition assignments displayed across family/child/enrollment/billing views, subsidy/agency tracking, Stripe Connect onboarding/status/refresh, card-first parent checkout with secure bank options, payment-method setup/requests, Thursday recurring tuition scheduling for the following week, autopay, payment reminders, dunning, reconciliation reports, school-scoped Stripe Terminal payments, terminal store checkout, and Kid City software invoices.
...
- Public survey responses require an active survey, persistent throttling, tenant-valid center/family references, serialized atomic rollup/audit writes, and a sanitized response that never returns prior comments or respondent details.
...
- **ProCare cutover:** run final import mapping against real exports from every active school, require zero unresolved account links and reviewed duplicate groups, and confirm every used field is mapped, transformed, or intentionally excluded.
...
- Historical rollout materials identified Longmont as a preferred early test location, but no school should be treated as approved for ProCare preview, import, payout activation, or cutover without a current dated decision.
- The July 24 Oakleaf and Canton preparation results remain blocked by unresolved authoritative-source links; see `PROCARE_EXPORT_VALIDATION_2026-07-24.md`.
