  ...
  const completeStepCount = localAssessment.confirmationCurrent
    ...
    : selectedPath
      ? localAssessment.canConfirm
        ? 3
        : data.evidence.familyCount || data.evidence.latestImportBatch
          ? 2
          : 1
      : 0;
  ...
  return (
    <section id="school-data-setup" className="scroll-mt-24 rounded-xl border bg-card/80 p-5 shadow-sm" aria-labelledby="school-data-setup-title">
            ...
            {selectedPath === "import_existing" ? (
              <div className="space-y-2">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {sourceSystem === "other"
                    ? "The BEE setup team prepares and verifies a school-specific mapping before the director uploads or confirms the package. Do not send an unreviewed provider export through the ProCare importer."
          ...
          <aside className="space-y-4">
            <div className="rounded-xl border bg-background/60 p-4">
              ...
              <dl className="mt-3 grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-xs text-muted-foreground">Families</dt><dd className="mt-1 font-semibold tabular-nums">{data.evidence.familyCount.toLocaleString()}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Children</dt><dd className="mt-1 font-semibold tabular-nums">{data.evidence.childCount.toLocaleString()}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Guardians</dt><dd className="mt-1 font-semibold tabular-nums">{data.evidence.guardianCount.toLocaleString()}</dd></div>
              ...
              {selectedPath === "start_clean" ? (
                <div className="mt-3 space-y-1 border-t pt-3 text-xs text-muted-foreground">
                  <p>{data.evidence.familiesMissingGuardianCount.toLocaleString()} current families missing a guardian</p>
            ...
            {localAssessment.canConfirm && !localAssessment.confirmationCurrent ? (
              <label className="flex min-h-11 items-start gap-3 rounded-xl border border-primary/30 bg-primary/5 p-4 text-sm">
                ...
                <span>
                  ...
                  <span className="mt-1 block text-xs leading-5 text-muted-foreground">I reviewed the school-scoped records and unresolved items, or confirmed that this new school has no current families yet. This does not activate invitations, billing, payments, kiosk access, or cutover.</span>
                </span>
              </label>
            ) : null}
      ...
      <div className="mt-5 flex flex-col gap-3 border-t pt-5 sm:flex-row sm:flex-wrap">
        ...
        {selectedPath === "import_existing" && sourceSystem === "procare" ? (
          hasUnsavedChanges ? (
        ...
        ) : selectedPath === "import_existing" && sourceSystem === "other" && !hasUnsavedChanges ? (
          <div className="flex min-h-10 items-center rounded-lg border border-emerald-500/25 bg-emerald-500/5 px-3 text-sm text-muted-foreground">
            Source handoff saved. The BEE setup team prepares the reviewed adapter before any upload.
          </div>
        ) : selectedPath === "start_clean" ? (
          hasUnsavedChanges ? (
        ...
        {selectedPath === "import_existing" && sourceSystem === "procare" && latestBatch && !hasUnsavedChanges ? (
