...
3. Reconcile family access in both directions against the school's approved guardian and authorized-pickup relationships. For every parent or pickup contact expected to sign in, match the active Prisma user, normalized email, Supabase Auth identity, correct `PARENT_GUARDIAN` or `AUTHORIZED_PICKUP` role, and exact `Guardian.userId` or `AuthorizedPickup.userId` link to the approved family and children. Verify each expected Auth account is not banned or disabled, has a usable password credential, and can meet its required MFA state; have the account holder prove sign-in where needed without sharing a password or factor. Record any unverified or unusable account as a parent/pickup-access GO blocker. Inventory every existing active parent/guardian **and authorized-pickup** application user and relationship link that could reach this school's family data, including accounts absent from the intended invitation list and former guardians or pickup contacts. Check active `DeviceSession` records and account `sessionVersion` independently of Auth login status; an Auth ban alone does not revoke an application cookie. For any removed or unapproved relationship, use the separate identity/session gate to invalidate older cookies by incrementing session version and revoking active device sessions, or prove expiry after untracked-cookie issuance has stopped; verify old access is denied. Hold parent/pickup-access GO for any unresolved account, role, relationship, or session exception. Do not change real identities or send invitations from this review.
4. Use only the approved `Parent Portal Access` invitation or resend action. A current, safely linked family can be invited without requiring a ProCare import batch; unresolved identity or relationship conflicts still block access.
5. Do not send a broad invitation wave without the named school's approval.
6. If invitations were already sent, review the completion record and any delivery failures or manual-copy fallbacks.
7. Have an approved test parent and an approved authorized-pickup account sign in; verify each sees only the intended family and role-appropriate child information before launch.
8. Tell parents to use the email and password in their welcome email. If they changed or forgot the password, they should use `Forgot password`.
9. Describe the Parent Portal as browser-installed: Safari `Add to Home Screen` on iPhone or iPad, Chrome `Install app` or `Add to Home screen` on Android, and the browser install option on desktop.
...
- [ ] Every staff member expected to sign in has a reconciled application user, usable password-login Auth account, approved application role, and exact school grant; every active Prisma user with potential school access and any outstanding application/device session has been checked against the approved roster regardless of Auth login state; older untracked cookies have been invalidated or their expiry proved; intentionally uninvited staff and access exceptions are named, owned, and held off
- [ ] Director access tested
- [ ] Teacher access and rosters tested
- [ ] Every expected parent/guardian and authorized-pickup account has usable password/MFA sign-in; every existing application account, role, family link, and outstanding session with potential school-family access is reconciled against the approved guardian/pickup roster; removed or unexpected access is denied before parent/pickup-access GO
...
- Bidirectional staff/account and application-session reconciliation evidence, intentionally uninvited staff, unexpected access, and blockers/owners:
- Bidirectional guardian/authorized-pickup account, role, family-link, and session reconciliation evidence, unexpected access, and blockers/owners:
