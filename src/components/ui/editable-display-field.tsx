...
export function EditableDisplayField({ id, label, value, placeholder, emptyLabel = "Not provided", multiline = false, inputMode = "text", type = "text", name, autoComplete, spellCheck, onChange, className }: Props) {
  ...
  if (editing) return (
    <div className={cn("space-y-2 rounded-lg border border-primary/35 bg-card p-3", className)}>
      ...
      {multiline
        ? <Textarea id={id} name={name} autoComplete={autoComplete} spellCheck={spellCheck} value={draft} onChange={(event) => updateDraft(event.target.value)} placeholder={placeholder} className="min-h-32" autoFocus />
        : <Input id={id} name={name} type={type} autoComplete={autoComplete} spellCheck={spellCheck} inputMode={inputMode} value={draft} onChange={(event) => updateDraft(event.target.value)} placeholder={placeholder} autoFocus onKeyDown={(event) => { if (event.key === "Enter") finish(); if (event.key === "Escape") cancel(); }} />}
    </div>
  );
  return (
    <button type="button" onClick={beginEditing} className={cn("group relative w-full overflow-hidden rounded-lg border border-border bg-card p-4 text-left hover:border-primary/40 hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", className)} aria-label={`Edit ${label}`}>
      ...
      <span className={cn("mt-2 block break-words whitespace-pre-wrap text-sm leading-6", value ? "font-medium text-foreground" : "italic text-muted-foreground")}>{value || emptyLabel}</span>
