...
- [ ] Keep browser source data at or below 3.5 MB; the application also verifies the complete request remains below 4 MB after confirmation metadata is added. ZIP the school's unchanged reports when needed; if the complete ZIP is still larger, use the file-only preflight and retain its review packet. Never remove a required report merely to reduce size.
