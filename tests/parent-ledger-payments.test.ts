...
test("parent invoice data and checkout do not expose or charge agency responsibility", () => {
  ...
  const snapshot = readFileSync("src/lib/family-payment-snapshot.ts", "utf8");
  const authority = readFileSync("src/lib/billing-actor-authorization.ts", "utf8");
  const service = readFileSync("src/lib/family-payment-service.ts", "utf8");
  ...
  assert.match(route, /parentCheckout = isParentGuardian\(user\) && !userCanManageBilling/);
  assert.match(authority, /tx.guardian.findFirst\(\{ where: \{ userId: actor.id, familyId: target.familyId/);
  assert.match(route, /readAuthorizedFamilyPaymentTarget\(tx, user/);
  assert.match(snapshot, /parentPaymentAmountCents\(/);
  assert.match(snapshot, /paymentCollectionResponsibilityHoldRequired\(/);
  assert.doesNotMatch(snapshot, /enforceCollectionHold:\s*true/);
  assert.match(route, /parent_account_payment_responsibility_review_required/);
  assert.match(route, /amountCents > fresh.collectableCents/);
  ...
  assert.match(service, /stripePaymentClaimConflict\(\{ scope: "family_balance", payment \}\)/);
  assert.match(service, /jsonRecord\(existing.customFields\).invoiceId\)\) return held/);
...
test("director billing routes agency remittances through the guarded claim workflow", () => {
  ...
  const agencyRoute = readFileSync("src/app/api/billing/agency-claims/route.ts", "utf8");
  ...
  assert.match(agencyRoute, /type: "agency_payment"/);
  assert.match(agencyRoute, /externalId: `agency-remittance:\$\{remittance\.id\}`/);
