...
3. Reconcile parent access in both directions against the school's approved family/guardian relationships. For every parent expected to sign in, match the active Prisma user, normalized email, Supabase Auth identity, and exact `Guardian.userId` links to the approved family and children. Inventory every existing active parent/guardian application user and `Guardian.userId` link that could reach this school's family data, including accounts absent from the intended invitation list and former guardians. Check active `DeviceSession` records and account `sessionVersion` independently of Auth login status; an Auth ban alone does not revoke an application cookie. For any removed or unapproved relationship, use the separate identity/session gate to invalidate older cookies by incrementing session version and revoking active device sessions, or prove expiry after untracked-cookie issuance has stopped; verify old access is denied. Hold parent-access GO for any unresolved account, relationship, or session exception. Do not change real identities or send invitations from this review.
...
7. Have an approved test parent sign in and verify family-only visibility before launch.
...
- [ ] Every existing parent/guardian application account, family link, and outstanding session with potential school-family access is reconciled against the approved guardian roster; removed or unexpected access is denied before parent-access GO
...
- Bidirectional parent/guardian account, family-link, and session reconciliation evidence, unexpected access, and blockers/owners:
