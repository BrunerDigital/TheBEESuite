...
- The client clears a draft only for the exact submitted receipt. Pending controls lock. Ordinary links/unload guard document, message, and contact drafts; browser Back and arbitrary programmatic routing are not claimed to be covered.
...
- Independent source screenshots under `output/playwright/parent-document-source-final/measurements` verify both signature/upload forms at 320/390px and 200% text. Earlier final-focus runs passed 48 Chromium and 48 WebKit density cases plus 72 rapid shortcut steps; fresh post-resume checks are in progress.
