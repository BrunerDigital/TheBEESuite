...
export const REPORT_DEFINITIONS: Record<ReportKind, { source: string; definition: string }> = {
  ...
  weekly_billing: { source: "BEE Suite invoices", definition: "Invoices created or due in the selected range, grouped into Monday-Sunday weeks in each center's local time. Open and overdue are the current status of those invoices, not an all-time as-of AR balance." },
  weekly_payments: { source: "BEE Suite payments", definition: "Successful payments paid in the selected range, grouped into Monday-Sunday weeks in each center's local time." },
